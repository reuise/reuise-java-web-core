...
public abstract class DefaultWebSvgPart<S extends DefaultWebSvgPart<S, O>, O extends WebSvgPartOptions> implements WebSvg , WebComponentPart {
    @Override
    ...
    protected DefaultWebSvgPart(O options) {
    }
    ...
    public S setWidth(Object width) {
        setAttribute("width", Style.parseProperty(width));
        return self();
    }
    ...
    public S setHeight(Object height) {
        setAttribute("height", Style.parseProperty(height));
        return self();
    }
    ...
    @Override
    public WebSvg getComponent() {
        return ((WebSvg) (baseComponentPart.getComponent()));
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        WebSvgPart component = ((WebSvgPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getXOption(), component::setX);
        applicator.add(options.getYOption(), component::setY);
        applicator.add(options.getViewBoxOption(), component::setViewBox);
    }
    ...
    public WebSvgPath addPath(String path) {
        WebSvgPathOptions pathOpts = getBaseComponentPart().getComponentFactory().createSvgPathOptions();
        pathOpts.setPath(path);
        return addPath(pathOpts);
    }
    ...
    public WebSvgPath addPath(String path, String fill, String stroke, double strokeWidth) {
        WebSvgPathOptions pathOpts = getBaseComponentPart().getComponentFactory().createSvgPathOptions();
        pathOpts.setPath(path);
        pathOpts.setFill(fill);
        pathOpts.setStroke(stroke);
        pathOpts.setStrokeWidth(strokeWidth);
        return addPath(pathOpts);
    }
    ...
    public WebSvgPath addPath(WebSvgPathOptions options) {
        WebSvgPath svgPath = getBaseComponentPart().getComponentFactory().createSvgPath(options);
        add(svgPath);
        return svgPath;
    }
}