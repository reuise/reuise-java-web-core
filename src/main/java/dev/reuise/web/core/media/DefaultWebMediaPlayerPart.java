...
import dev.reuise.core.media.DefaultCoreMediaPlayerPart;
...
public abstract class DefaultWebMediaPlayerPart<S extends DefaultWebMediaPlayerPart<S, O>, O extends WebMediaPlayerPartOptions> extends DefaultCoreMediaPlayerPart<S, O> implements WebMediaPlayer , WebComponentPart {
    @Override
    ...
    public S setAutoplay(Boolean autoplay) {
        if (Boolean.FALSE.equals(autoplay))
    ...
    public boolean isAutoplay() {
        ...
        return autoplayAttr != null;
    }

    @Override
    public S setMuted(Boolean muted) {
        if (Boolean.FALSE.equals(muted))
    ...
    public boolean isMuted() {
        ...
        return mutedAttr != null;
    }

    @Override
    public S setControls(Boolean controls) {
        if (Boolean.FALSE.equals(controls))
    ...
    public boolean isControls() {
        String controlsAttr = getAttribute("controls");
        return controlsAttr != null;
    }
    ...
    public boolean isLoop() {
        ...
        return loopAttr != null;
    }

    @Override
    public S add(CoreComponent child) {
        if (child instanceof CoreTextTrack)
            return addTextTrack(((CoreTextTrack) (child)));
    ...
    public void remove(CoreComponent child) {
        if (child instanceof CoreTextTrack)
            removeTextTrack(((CoreTextTrack) (child)));

        getParentComponentPart().remove(child);
    }

    @Override
    public S addTextTrack(CoreTextTrack textTrack) {
        getParentComponentPart().add(textTrack);
        return self();
    }

    @Override
    public S removeTextTrack(CoreTextTrack textTrack) {
        getParentComponentPart().remove(textTrack);
        return self();
    }
    ...
    protected DefaultWebMediaPlayerPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebMediaPlayer getComponent() {
        return ((WebMediaPlayer) (baseComponentPart.getComponent()));
    }
