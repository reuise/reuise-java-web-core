...
import dev.reuise.core.media.DefaultCoreTextTrackPart;
import dev.reuise.core.media.TextTrackKind;
import dev.reuise.core.media.TextTrackMode;
...
public abstract class DefaultWebTextTrackPart<S extends DefaultWebTextTrackPart<S, O>, O extends WebTextTrackPartOptions> extends DefaultCoreTextTrackPart<S, O> implements WebTextTrack , WebComponentPart {
    @Override
    ...
    public S setKind(TextTrackKind kind) {
        if (kind != null)
            setAttribute("kind", kind.name().toLowerCase());
    ...
    public TextTrackKind getKind() {
        String kindValue = getAttribute("kind");
        if (kindValue == null)
            return null;

        return TextTrackKind.valueOf(kindValue.toUpperCase());
    }

    @Override
    public S setMode(TextTrackMode mode) {
        if (mode != null)
            setAttribute("mode", mode.name().toLowerCase());
    ...
    public TextTrackMode getMode() {
        String modeValue = getAttribute("mode");
        if (modeValue == null)
            return null;

        return TextTrackMode.valueOf(modeValue.toUpperCase());
    }
    ...
    protected DefaultWebTextTrackPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebTextTrack getComponent() {
        return ((WebTextTrack) (baseComponentPart.getComponent()));
    }
