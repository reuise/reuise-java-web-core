...
import dev.reuise.core.input.DefaultCorePasswordFieldPart;
import dev.reuise.core.input.PasswordInputValidator;
...
public abstract class DefaultWebPasswordFieldPart<S extends DefaultWebPasswordFieldPart<S, O>, O extends WebPasswordFieldPartOptions> extends DefaultCorePasswordFieldPart<S, O> implements WebPasswordField , WebComponentPart {
    @Override
    public S setPasswordMode(PasswordMode passwordMode) {
        super.setPasswordMode(passwordMode);
        getValidators(PasswordInputValidator.class).forEach(v -> v.setPasswordMode(passwordMode));
        getInput().setAutocomplete(passwordMode == PasswordMode.NEW_PASSWORD ? "new-password" : "current-password");
        return self();
    }

    @Override
    public PasswordMode getPasswordMode() {
        String autocomplete = getInput().getAutocomplete();
        return autocomplete.equals("new-password") ? PasswordMode.NEW_PASSWORD : PasswordMode.CURRENT_PASSWORD;
    }
    ...
    protected DefaultWebPasswordFieldPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebPasswordField getComponent() {
        return ((WebPasswordField) (baseComponentPart.getComponent()));
    }
