...
import dev.reuise.core.input.DefaultCoreSearchFieldPart;
...
public abstract class DefaultWebSearchFieldPart<S extends DefaultWebSearchFieldPart<S, O>, O extends WebSearchFieldPartOptions> extends DefaultCoreSearchFieldPart<S, O> implements WebSearchField , WebComponentPart {
    ...
    protected DefaultWebSearchFieldPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebSearchField getComponent() {
        return ((WebSearchField) (baseComponentPart.getComponent()));
    }
    ...
    public boolean onPreInitialize(O options) {
        ...
        if (!isRehydrated()) {
            ...
            WebSvg svg = startIcon.getSvg();
            WebImage img = startIcon.getImage();
            if ((svg != null) && (img == null)) {
                WebSvgPathOptions svgPathOpts = getComponentFactory().createSvgPathOptions();
                svgPathOpts.setPath("M20.49,19l-5.73-5.73C15.53,12.2,16,10.91,16,9.5C16,5.91,13.09,3,9.5,3S3,5.91,3,9.5C3,13.09,5.91,16,9.5,16 c1.41,0,2.7-0.47,3.77-1.24L19,20.49L20.49,19z M5,9.5C5,7.01,7.01,5,9.5,5S14,7.01,14,9.5S11.99,14,9.5,14S5,11.99,5,9.5z");
                svg.add(getComponentFactory().createSvgPath(svgPathOpts));
            }
        }
        return true;
    }
