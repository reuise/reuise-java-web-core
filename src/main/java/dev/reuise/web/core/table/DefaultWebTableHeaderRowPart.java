...
import dev.reuise.core.table.DefaultCoreTableHeaderRowPart;
...
public abstract class DefaultWebTableHeaderRowPart<S extends DefaultWebTableHeaderRowPart<S, O>, O extends WebTableHeaderRowPartOptions> extends DefaultCoreTableHeaderRowPart<S, O> implements WebTableHeaderRow , WebComponentPart {
    @Override
    public S addColumn(CoreTableColumnOptions column) {
        if (column == null)
            return self();

        WebTableHeaderCellOptions headerRowCellOpts = getComponentFactory().createTableHeaderCellOptions();
        headerRowCellOpts.setLabel(column.getLabel());
        headerRowCellOpts.setMenuOptions(column.getMenuOptions());
        headerRowCellOpts.setMenuButtonOptions(column.getMenuButtonOptions());
        WebTableColumnOptions webColumn = ((WebTableColumnOptions) (column));
        headerRowCellOpts.setStyleClasses(webColumn.getStyleClasses());
        add(getComponentFactory().createTableHeaderCell(headerRowCellOpts));
        return self();
    }
    ...
    protected DefaultWebTableHeaderRowPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebTableHeaderRow getComponent() {
        return ((WebTableHeaderRow) (baseComponentPart.getComponent()));
    }
