...
import dev.reuise.core.layout.DefaultCoreFieldSetPart;
...
public abstract class DefaultWebFieldSetPart<S extends DefaultWebFieldSetPart<S, O>, O extends WebFieldSetPartOptions> extends DefaultCoreFieldSetPart<S, O> implements WebFieldSet , WebComponentPart {
    private WebLabel labelContainer;
    ...
    public S setLabel(String label) {
        ...
        labelContainer.setVisible((label != null) && (!label.isEmpty()));
    ...
    protected DefaultWebFieldSetPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebFieldSet getComponent() {
        return ((WebFieldSet) (baseComponentPart.getComponent()));
    }
    ...
    public boolean onPreInitialize(O options) {
        ...
        WebLabelOptions labelOpts = getComponentFactory().createLabelOptions();
        if (!isRehydrated()) {
            labelOpts.setTagName("legend");
            labelOpts.setText(options.getLabel());
        } else
            labelOpts.setRehydration("legend", this);

        labelContainer = getComponentFactory().createLabel(labelOpts);
        add(labelContainer);
        return true;
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-fieldset legend:empty").setDisplay("none");
    }
}