...
import dev.reuise.core.layout.DefaultCoreSurfacePart;
...
public abstract class DefaultWebSurfacePart<S extends DefaultWebSurfacePart<S, O>, O extends WebSurfacePartOptions> extends DefaultCoreSurfacePart<S, O> implements WebComponentPart , WebSurface {
    @Override
    public S setType(SurfaceType surfacetype) {
        if (surfacetype == null) {
            removeSurfaceTypeStyleClass();
            return self();
        }
        for (SurfaceType s : SurfaceType.values()) {
            addOrRemoveStyleClass(createSurfaceTypeStyleClassName(s), surfacetype == s);
        }
        return self();
    }
    ...
    public SurfaceType getType() {
        for (SurfaceType t : SurfaceType.values()) {
            if (hasStyleClass(createSurfaceTypeStyleClassName(t)))
                return t;
    ...
    private void removeSurfaceTypeStyleClass() {
        for (SurfaceType t : SurfaceType.values())
            removeStyleClass(createSurfaceTypeStyleClassName(t));

    }

    private String createSurfaceTypeStyleClassName(SurfaceType surfacetype) {
        return (getBaseStyleClass() + "--") + surfacetype.getStyleName();
    }
    ...
    protected DefaultWebSurfacePart(O options) {
        super(options);
    }
    ...
    @Override
    public WebSurface getComponent() {
        return ((WebSurface) (baseComponentPart.getComponent()));
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-surface").setDisplay("flex").setBorderRadius(12);
        commonStyles.addRule(".reuise-surface--filled");
        commonStyles.addRule(".reuise-surface--elevated").setBoxShadow("0px 1px 3px 1px rgba(0, 0, 0, 0.15), 0px 1px 2px 0px rgba(0, 0, 0, 0.30)");
        commonStyles.addRule(".reuise-surface--outlined").setBorder("1px solid #CAC4D0");
    }
}