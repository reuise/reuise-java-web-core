...
import dev.reuise.core.skeleton.DefaultCoreSkeletonPart;
...
public abstract class DefaultWebSkeletonPart<S extends DefaultWebSkeletonPart<S, O>, O extends WebSkeletonPartOptions> extends DefaultCoreSkeletonPart<S, O> implements WebComponentPart , WebSkeleton {
    private void removeVariantStyleClass() {
        for (SkeletonShape v : SkeletonShape.values())
            removeStyleClass(createVariantStyleClassName(v));

    }

    private String createVariantStyleClassName(SkeletonShape variant) {
        return (getBaseStyleClass() + "--") + variant.getShortName();
    }

    @Override
    public S setShape(SkeletonShape variant) {
        if (variant == null) {
            removeVariantStyleClass();
            return self();
        }
        for (SkeletonShape v : SkeletonShape.values()) {
            addOrRemoveStyleClass(createVariantStyleClassName(v), variant == v);
        }
        return self();
    }
    ...
    public SkeletonShape getShape() {
        for (SkeletonShape v : SkeletonShape.values()) {
            if (hasStyleClass(createVariantStyleClassName(v)))
                return v;
    ...
    protected DefaultWebSkeletonPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebSkeleton getComponent() {
        return ((WebSkeleton) (baseComponentPart.getComponent()));
    }
