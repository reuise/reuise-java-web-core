...
import dev.reuise.core.splitcontainer.DefaultCoreSplitContainerPanelPart;
...
public abstract class DefaultWebSplitContainerPanelPart<S extends DefaultWebSplitContainerPanelPart<S, O>, O extends WebSplitContainerPanelPartOptions> extends DefaultCoreSplitContainerPanelPart<S, O> implements WebComponentPart , WebSplitContainerPanel {
    @Override
    ...
    public S setCollapsed(Boolean collapsed, State state) {
        addOrRemoveStyleClass("reuise-splitcontainer_panel--collapsed" + (state == null ? "" : "_" + state.getStateKey()), collapsed);
        return self();
    }
    ...
    public S setSize(Object size, State state) {
        ...
        if (direction == SplitDirection.VERTICAL)
            style.setWidth(Style.parseProperty(size), true);
        else if (direction == SplitDirection.HORIZONTAL)
            style.setHeight(Style.parseProperty(size), true);
    ...
    public S setMinSize(Object minSize, State state) {
        ...
        Style style = getStyle(state);
        if (direction == SplitDirection.VERTICAL)
            style.setMinWidth(Style.parseProperty(minSize), true);
        else if (direction == SplitDirection.HORIZONTAL)
            style.setMinHeight(Style.parseProperty(minSize), true);
    ...
    public S setMaxSize(Object maxSize, State state) {
        ...
        Style style = getStyle(state);
        if (direction == SplitDirection.VERTICAL)
            style.setMaxWidth(Style.parseProperty(maxSize), true);
        else if (direction == SplitDirection.HORIZONTAL)
            style.setMaxHeight(Style.parseProperty(maxSize), true);
    ...
    protected DefaultWebSplitContainerPanelPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebSplitContainerPanel getComponent() {
        return ((WebSplitContainerPanel) (baseComponentPart.getComponent()));
    }
    ...
    public S setCollapsedAllStates(Boolean collapsed) {
        setCollapsed(ScreenSizeValues.of(collapsed, collapsed));
    ...
    public S setMinSizeAllStates(Object minSize) {
        setMinSize(ScreenSizeValues.of(minSize, minSize));
        setMinSize(minSize);
        return self();
    }
    ...
    public S setMaxSizeAllStates(Object maxSize) {
        setMaxSize(ScreenSizeValues.of(maxSize, maxSize));
        setMaxSize(maxSize);
        return self();
    }
