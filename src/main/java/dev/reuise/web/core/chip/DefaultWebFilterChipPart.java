...
import dev.reuise.core.chip.DefaultCoreFilterChipPart;
...
public abstract class DefaultWebFilterChipPart<S extends DefaultWebFilterChipPart<S, O>, O extends WebFilterChipPartOptions> extends DefaultCoreFilterChipPart<S, O> implements WebComponentPart , WebFilterChip {
    @Override
    public boolean isSelected() {
        return hasStyleClass(getBaseStyleClass() + "--selected");
    }

    @Override
    public S setSelected(Boolean selected) {
        addOrRemoveStyleClass(getBaseStyleClass() + "--selected", selected);
        return self();
    }

    @Override
    public S setHasCheckIcon(Boolean hasCheckIcon) {
        addOrRemoveStyleClass(getBaseStyleClass() + "--hascheckicon", hasCheckIcon);
        return self();
    }

    @Override
    public boolean isHasCheckIcon() {
        return hasStyleClass(getBaseStyleClass() + "--hascheckicon");
    }

    @Override
    public S setHasRemoveIcon(Boolean hasRemoveIcon) {
        addOrRemoveStyleClass(getBaseStyleClass() + "--hasremoveicon", hasRemoveIcon);
        return self();
    }

    @Override
    public boolean isHasRemoveIcon() {
        return hasStyleClass(getBaseStyleClass() + "--hasremoveicon");
    }

    private WebChipPart chipPart;

    private WebParentComponentPart parentComponentPart;

    private WebBaseComponentPart baseComponentPart;

    protected DefaultWebFilterChipPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebFilterChip getComponent() {
        return ((WebFilterChip) (baseComponentPart.getComponent()));
    }
    ...
    public void onInitialize(O options) {
        ...
        if (!isRehydrated()) {
            if (isHasCheckIcon()) {
                WebIcon startIcon = getStartIcon();
                if (startIcon != null) {
                    WebSvg startIconSvg = startIcon.getSvg();
                    if (startIconSvg != null) {
                        WebSvgPathOptions svgPathOpts = getComponentFactory().createSvgPathOptions();
                        svgPathOpts.setPath("M6.75012 12.1274L3.62262 8.99988L2.55762 10.0574L6.75012 14.2499L15.7501 5.24988L14.6926 4.19238L6.75012 12.1274Z");
                        WebSvgPath svgPath = getComponentFactory().createSvgPath(svgPathOpts);
                        startIconSvg.add(svgPath);
                    }
                }
            }
            if (isHasRemoveIcon()) {
                WebIcon endIcon = getEndIcon();
                if (endIcon != null) {
                    WebSvg endIconSvg = endIcon.getSvg();
                    if (endIconSvg != null) {
                        WebSvgPathOptions svgPathOpts = getComponentFactory().createSvgPathOptions();
                        svgPathOpts.setPath("m249 849-42-42 231-231-231-231 42-42 231 231 231-231 42 42-231 231 231 231-42 42-231-231-231 231Z");
                        WebSvgPath svgPath = getComponentFactory().createSvgPath(svgPathOpts);
                        endIconSvg.add(svgPath);
                    }
                }
            }
        }
