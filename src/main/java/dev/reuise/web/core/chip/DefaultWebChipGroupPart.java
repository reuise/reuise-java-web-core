...
import dev.reuise.core.chip.DefaultCoreChipGroupPart;
...
public abstract class DefaultWebChipGroupPart<S extends DefaultWebChipGroupPart<S, O>, O extends WebChipGroupPartOptions> extends DefaultCoreChipGroupPart<S, O> implements WebComponentPart , WebChipGroup {
    @Override
    public S setHideUnselected(Boolean hideUnselected) {
        addOrRemoveStyleClass(getBaseStyleClass() + "--hideunselected", hideUnselected);
        return self();
    }

    @Override
    public boolean isHideUnselected() {
        return hasStyleClass(getBaseStyleClass() + "--hideunselected");
    }

    private WebFieldSetPart fieldSetPart;
    ...
    protected DefaultWebChipGroupPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebChipGroup getComponent() {
        return ((WebChipGroup) (baseComponentPart.getComponent()));
    }
    ...
    public void onInitialize(O options) {
        ...
        if (!isRehydrated()) {
            WebChip addButton = getAddButton();
            if (addButton == null)
                return;

            WebIcon startIcon = addButton.getStartIcon();
            if (startIcon == null)
                return;
            ...
            svgPathOpts.setPath("M440-440H200v-80h240v-240h80v240h240v80H520v240h-80v-240Z");
    ...
    @Override
    public WebChip getAddButton() {
        return ((WebChip) (super.getAddButton()));
    }
