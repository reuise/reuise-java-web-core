...
import dev.reuise.core.badge.DefaultCoreBadgePart;
...
public abstract class DefaultWebBadgePart<S extends DefaultWebBadgePart<S, O>, O extends WebBadgePartOptions> extends DefaultCoreBadgePart<S, O> implements WebBadge , WebComponentPart {
    private WebBaseComponentPart baseComponentPart;

    protected DefaultWebBadgePart(O options) {
        super(options);
    }
    ...
    @Override
    public WebBadge getComponent() {
        return ((WebBadge) (baseComponentPart.getComponent()));
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-badge").setBackgroundColor("#C84950").setWidth("auto").setWhiteSpace("nowrap").setOverflow("hidden").setMinWidth("20px").setHeight("20px").setBorderRadius("50%").setFontSize("11px").setTextAlign("center");
    }
}