...
public abstract class DefaultWebSlideContainerPart<S extends DefaultWebSlideContainerPart<S, O>, O extends WebSlideContainerPartOptions> extends DefaultCoreSlideContainerPart<S, O> implements WebSlideContainer , WebComponentPart {
    private static final String OVERFLOW_VERTICAL = "hidden slide";

    private static final String OVERFLOW_HORIZONTAL = "slide hidden";

    private WebSlideContainerEdge topEdge;

    private WebSlideContainerEdge bottomEdge;

    private WebSlideContainerEdge leftEdge;
    ...
    public SlideDirection getSlideDirection() {
        ...
        if (overflow.equalsIgnoreCase(OVERFLOW_HORIZONTAL))
            return SlideDirection.HORIZONTAL;
    ...
    public S setSlideDirection(SlideDirection slideDirection) {
        ...
        if (slideDirection == SlideDirection.VERTICAL) {
            ...
            slideAreaStyle.setOverflow(OVERFLOW_HORIZONTAL);
        }
        return self();
    }
    ...
    private void initHorizontalEdge() {
        if (leftEdge == null) {
            leftEdge = createEdge("left", getSlideAreaBefore());
        }
        if (rightEdge == null) {
            rightEdge = createEdge("right", getSlideAreaAfter());
        }
    }

    private void initVerticalEdge() {
        if (topEdge == null) {
            topEdge = createEdge("top", getSlideAreaBefore());
        }
        if (bottomEdge == null) {
            bottomEdge = createEdge("bottom", getSlideAreaAfter());
        }
    }

    private WebSlideContainerEdge createEdge(String direction, CoreContainer parent) {
        WebSlideContainerEdgeOptions edgeOpts = getBaseComponentPart().getComponentFactory().createSlideContainerEdgeOptions();
        if (!isRehydrated())
            edgeOpts.setBaseStyleClass("reuise-slidecontainer_edge").addStyleClass(("reuise-slidecontainer_" + direction) + "edge");
        else
    ...
    public WebSlideContainerEdge getTopEdge() {
        return topEdge;
    }

    public WebSlideContainerEdge getBottomEdge() {
        return bottomEdge;
    }

    public WebSlideContainerEdge getLeftEdge() {
        return leftEdge;
    }

    public WebSlideContainerEdge getRightEdge() {
        return rightEdge;
    }

    @Override
    public S setShowSlideBars(Boolean show) {
        return self();
    }
    ...
    protected DefaultWebSlideContainerPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (slideAreaBefore != null)
            slideAreaBefore.setRootComponent(rootComponent);

        if (slideArea != null)
            slideArea.setRootComponent(rootComponent);

        if (slideAreaAfter != null)
            slideAreaAfter.setRootComponent(rootComponent);
    ...
    public S setTheme(Theme theme) {
        ...
        if (slideAreaBefore != null)
            slideAreaBefore.setTheme(theme);

        if (slideArea != null)
            slideArea.setTheme(theme);

        if (slideAreaAfter != null)
            slideAreaAfter.setTheme(theme);
    ...
    @Override
    public WebSlideContainer getComponent() {
        return ((WebSlideContainer) (baseComponentPart.getComponent()));
    }
    ...
    @Override
    public WebContainer getSlideAreaBefore() {
        return ((WebContainer) (super.getSlideAreaBefore()));
    }

    @Override
    public WebContainer getSlideArea() {
        return ((WebContainer) (super.getSlideArea()));
    }

    @Override
    public WebContainer getSlideAreaAfter() {
        return ((WebContainer) (super.getSlideAreaAfter()));
    }
