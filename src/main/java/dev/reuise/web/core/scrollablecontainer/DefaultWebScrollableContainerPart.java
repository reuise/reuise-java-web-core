...
public abstract class DefaultWebScrollableContainerPart<S extends DefaultWebScrollableContainerPart<S, O>, O extends WebScrollableContainerPartOptions> extends DefaultCoreScrollableContainerPart<S, O> implements WebScrollableContainer , WebComponentPart {
    private WebScrollableContainerEdge topEdge;

    private WebScrollableContainerEdge bottomEdge;

    private WebScrollableContainerEdge leftEdge;

    private WebScrollableContainerEdge rightEdge;

    private void initHorizontalEdge() {
        if (leftEdge == null) {
            leftEdge = createEdge("left", getScrollAreaBefore());
        }
        if (rightEdge == null) {
            rightEdge = createEdge("right", getScrollAreaAfter());
        }
    }

    private void initVerticalEdge() {
        if (topEdge == null) {
            topEdge = createEdge("top", getScrollAreaBefore());
        }
        if (bottomEdge == null) {
            bottomEdge = createEdge("bottom", getScrollAreaAfter());
        }
    }

    private WebScrollableContainerEdge createEdge(String direction, CoreContainer parent) {
        WebScrollableContainerEdgeOptions edgeOpts = getBaseComponentPart().getComponentFactory().createScrollableContainerEdgeOptions();
        if (!isRehydrated())
            edgeOpts.setBaseStyleClass("reuise-scrollablecontainer_edge").addStyleClass(("reuise-scrollablecontainer_" + direction) + "edge");
        else
    ...
    public WebScrollableContainerEdge getTopEdge() {
        return topEdge;
    }

    public WebScrollableContainerEdge getBottomEdge() {
        return bottomEdge;
    }

    public WebScrollableContainerEdge getLeftEdge() {
        return leftEdge;
    }

    public WebScrollableContainerEdge getRightEdge() {
        return rightEdge;
    }

    @Override
    public S setShowScrollBars(Boolean show) {
        return self();
    }
    ...
    protected DefaultWebScrollableContainerPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (scrollAreaBefore != null)
            scrollAreaBefore.setRootComponent(rootComponent);

        if (scrollArea != null)
            scrollArea.setRootComponent(rootComponent);

        if (scrollAreaAfter != null)
            scrollAreaAfter.setRootComponent(rootComponent);
    ...
    public S setTheme(Theme theme) {
        ...
        if (scrollAreaBefore != null)
            scrollAreaBefore.setTheme(theme);

        if (scrollArea != null)
            scrollArea.setTheme(theme);

        if (scrollAreaAfter != null)
            scrollAreaAfter.setTheme(theme);
    ...
    @Override
    public WebScrollableContainer getComponent() {
        return ((WebScrollableContainer) (baseComponentPart.getComponent()));
    }
    ...
    public void onInitialize(O options) {
        ...
        if (scrollDir != null) {
            if (scrollDir != ScrollDirection.NONE)
                addStyleClass("reuise-scrollablecontainer--scroll-" + scrollDir.name().toLowerCase());
    ...
    @Override
    public WebContainer getScrollAreaBefore() {
        return ((WebContainer) (super.getScrollAreaBefore()));
    }

    @Override
    public WebScrollableContainerScrollArea getScrollArea() {
        return ((WebScrollableContainerScrollArea) (super.getScrollArea()));
    }

    @Override
    public WebContainer getScrollAreaAfter() {
        return ((WebContainer) (super.getScrollAreaAfter()));
    }

    // Implementation
    // Implementation
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-scrollablecontainer").setDisplay("flex");
        commonStyles.addRule(".reuise-scrollablecontainer--scroll-vertical").setFlexDirection("column");
        commonStyles.addRule(".reuise-scrollablecontainer--scroll-horizontal").setFlexDirection("row");
        commonStyles.addRule(".reuise-scrollablecontainer_scrollarea_before, .reuise-scrollablecontainer_scrollarea_after").setPosition("relative");
        commonStyles.addRule(".reuise-scrollablecontainer_edge").setZIndex(9).setPointerEvents("none").setDisplay("flex");
        commonStyles.addRule(".reuise-scrollablecontainer_edge_scrollbutton").setPosition("absolute").setPointerEvents("all").setCursor("pointer").setWidth("32px").setHeight("32px");
        commonStyles.addRule(".reuise-scrollablecontainer_edge_scrollbutton:disabled,.reuise-scrollablecontainer_edge_scrollbutton.reuise-disabled").setOpacity("0");
        commonStyles.addRule(".reuise-scrollablecontainer_leftedge,.reuise-scrollablecontainer_rightedge").setPosition("absolute").setWidth("50px").setHeight("inherit").setTop("0").setBottom("0");
        commonStyles.addRule(".reuise-scrollablecontainer_rightedge .reuise-scrollablecontainer_edge_scrollbutton").setMarginRight("-32px");
        commonStyles.addRule(".reuise-scrollablecontainer_topedge .reuise-scrollablecontainer_edge_scrollbutton").setMarginTop("-32px");
        commonStyles.addRule(".reuise-scrollablecontainer_topedge,.reuise-scrollablecontainer_bottomedge").setPosition("absolute").setHeight("50px").setWidth("inherit").setLeft("0").setRight("0").setFlexDirection("row").setJustifyContent("center");
        commonStyles.addRule(".reuise-scrollablecontainer_rightedge,.reuise-scrollablecontainer_bottomedge").setRight("0").setDisplay("flex").setJustifyContent("end");
    }
}