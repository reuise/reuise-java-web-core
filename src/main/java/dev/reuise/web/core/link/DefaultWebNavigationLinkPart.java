...
import dev.reuise.core.link.DefaultCoreNavigationLinkPart;
...
import dev.reuise.web.core.view.AntPathMatcher;
...
public abstract class DefaultWebNavigationLinkPart<S extends DefaultWebNavigationLinkPart<S, O>, O extends WebNavigationLinkPartOptions> extends DefaultCoreNavigationLinkPart<S, O> implements WebNavigationLink , WebComponentPart {
    private AntPathMatcher matcher = new AntPathMatcher();

    @Override
    public S setActive(Boolean active) {
        addOrRemoveStyleClass("reuise-navigationlink--active", active);
        return self();
    }

    @Override
    public boolean isActive() {
        return hasStyleClass("reuise-navigationlink--active");
    }

    protected Boolean checkPathMatch(String urlPath, String linkPath) {
        ...
        switch (this.activeMatchType) {
            ...
            case PREFIX :
                String prefix = (linkPath.endsWith("/")) ? linkPath : linkPath + "/";
                return urlPath.startsWith(prefix);
            case PATTERN :
                boolean exact = (urlPath.equals(linkPath) || urlPath.equals(linkPath + "/")) || (urlPath + "/").equals(linkPath);
                if (exact)
                    return true;

                String normalized = (linkPath.endsWith("/")) ? linkPath + "**" : linkPath + "**";
                return matcher.match(normalized, urlPath);
            default :
                return false;
        }
    }

    private WebLinkPart linkPart;

    private WebParentComponentPart parentComponentPart;

    private WebBaseComponentPart baseComponentPart;

    protected PathMatchType activeMatchType;

    protected DefaultWebNavigationLinkPart(O options) {
        super(options);
    }
    ...
    public boolean isUseHistoryApi() {
        return linkPart.isUseHistoryApi();
    }

    @Override
    public S setUseHistoryApi(Boolean useHistoryApi) {
        this.linkPart.setUseHistoryApi(useHistoryApi);
        return self();
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        this.linkPart.setRootComponent(rootComponent);
    }
    ...
    @Override
    public WebNavigationLink getComponent() {
        return ((WebNavigationLink) (baseComponentPart.getComponent()));
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        ...
        WebNavigationLinkPart component = ((WebNavigationLinkPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getActiveMatchTypeOption(), component::setActiveMatchType);
    }
