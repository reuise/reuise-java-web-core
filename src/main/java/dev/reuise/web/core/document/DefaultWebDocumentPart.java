...
public abstract class DefaultWebDocumentPart<S extends DefaultWebDocumentPart<S, O>, O extends WebDocumentPartOptions> implements WebDocument , WebComponentPart {
    protected StyleBuilder commonStyles;

    @Override
    public StyleBuilder getCommonStyles() {
        return commonStyles;
    }
    ...
    @Override
    public S setStyleSheet(List<StyleSheet> styleSheets) {
        return self();
    }
    ...
    protected WebDocumentBody body;

    private final List<Class> componentTypeRegister = new ArrayList();

    protected DefaultWebDocumentPart(O options) {
        commonStyles = new StyleBuilder("reuise-commonstyles");
    }
    ...
    @Override
    public WebDocument getComponent() {
        return ((WebDocument) (baseComponentPart.getComponent()));
    }
    ...
    public WebDocumentBody getBody() {
        return body;
    }
    ...
    private WebDocumentBody createBody(O options) {
        if ((body != null) || (options == null))
            return body;

        WebDocumentBodyOptions bodyOptions = options.getBodyOptions();
        if (bodyOptions == null)
            return body;

        bodyOptions.setParent(this);
        ComponentCreator bodyCreator = options.getBodyCreator();
        body = (bodyCreator != null) ? ((WebDocumentBody) (bodyCreator.create(bodyOptions))) : getComponentFactory().createDocumentBody(bodyOptions);
        getParentComponentPart().add(body);
        return body;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        WebDocumentPart component = ((WebDocumentPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getTitleOption(), component::setTitle);
        applicator.add(options.getViewIdOption(), component::setViewId);
        applicator.add(options.getDescriptionOption(), component::setDescription);
        applicator.add(options.getKeywordsOption(), component::setKeywords);
        applicator.add(options.getThemeColorOption(), component::setThemeColor);
        applicator.add(options.getLanguageOption(), component::setLanguage);
        applicator.add(options.getPathOption(), component::setPath);
        applicator.add(options.getUrlOption(), component::setUrl);
        applicator.add(options.getCharsetOption(), component::setCharset);
        applicator.add(options.getStyleSheetOption(), component::setStyleSheet);
        applicator.add(options.getModelOption(), component::setModel);
    }
    ...
    // Implementation
    public void openDialog(CoreDialog dialog) {
        add(dialog);
        dialog.open();
    }

    // Implementation
    public boolean isViewId(String viewId) {
        if (viewId == null)
            return false;

        return viewId.equals(getViewId());
    }

    @Override
    public boolean registerComponentType(CoreComponent component) {
        if (isComponentTypeRegistered(component.getClass()))
            return false;

        component.onInitializeComponentType(self());
        componentTypeRegister.add(component.getClass());
        return true;
    }

    @Override
    public boolean isComponentTypeRegistered(Class<? extends CoreComponent> componentType) {
        return componentTypeRegister.contains(componentType);
    }
}