...
import dev.reuise.core.divider.DefaultCoreDividerPart;
import dev.reuise.core.divider.DividerDirection;
...
public abstract class DefaultWebDividerPart<S extends DefaultWebDividerPart<S, O>, O extends WebDividerPartOptions> extends DefaultCoreDividerPart<S, O> implements WebDivider , WebComponentPart {
    ...
    public S setLabelAlignment(Alignment labelAlignment) {
        if (labelAlignment == null) {
            removeLabelAlignmentStyleClass();
            return self();
        }
        for (Alignment s : Alignment.values()) {
            addOrRemoveStyleClass(createLabelAlignmentStyleClassName(s), labelAlignment == s);
        }
        return self();
    }
    ...
    public Alignment getLabelAlignment() {
        for (Alignment s : Alignment.values()) {
            if (hasStyleClass(createLabelAlignmentStyleClassName(s)))
                return s;
    ...
    private void removeLabelAlignmentStyleClass() {
        for (Alignment s : Alignment.values())
            removeStyleClass(createLabelAlignmentStyleClassName(s));

    }

    private String createLabelAlignmentStyleClassName(Alignment labelAlignment) {
        return (getBaseStyleClass() + "--") + labelAlignment.name().toLowerCase();
    }

    @Override
    public S setDirection(DividerDirection direction) {
        if (direction == null) {
            removeDirectionStyleClass();
            return self();
        }
        for (DividerDirection s : DividerDirection.values()) {
            addOrRemoveStyleClass(createDirectionStyleClassName(s), direction == s);
        }
        return self();
    }
    ...
    public DividerDirection getDirection() {
        for (DividerDirection s : DividerDirection.values()) {
            if (hasStyleClass(createDirectionStyleClassName(s)))
                return s;
    ...
    private void removeDirectionStyleClass() {
        for (DividerDirection s : DividerDirection.values())
            removeStyleClass(createDirectionStyleClassName(s));

    }

    private String createDirectionStyleClassName(DividerDirection direction) {
        return (getBaseStyleClass() + "--") + direction.name().toLowerCase();
    }
    ...
    protected DefaultWebDividerPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (startLine != null)
            startLine.setRootComponent(rootComponent);

        if (labelContainer != null)
            labelContainer.setRootComponent(rootComponent);

        if (endLine != null)
            endLine.setRootComponent(rootComponent);
    ...
    public S setTheme(Theme theme) {
        ...
        if (startLine != null)
            startLine.setTheme(theme);

        if (labelContainer != null)
            labelContainer.setTheme(theme);

        if (endLine != null)
            endLine.setTheme(theme);
    ...
    @Override
    public WebDivider getComponent() {
        return ((WebDivider) (baseComponentPart.getComponent()));
    }
    ...
    @Override
    public WebContainer getStartLine() {
        return ((WebContainer) (super.getStartLine()));
    }
    ...
    @Override
    public WebContainer getEndLine() {
        return ((WebContainer) (super.getEndLine()));
    }

    // Implementation
    // Implementation
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-divider").setDisplay("flex");
        commonStyles.addRule(".reuise-divider--horizontal").setFlexDirection("row").setPadding("16px 0").setAlignItems("center");
        commonStyles.addRule(".reuise-divider--vertical").setFlexDirection("column").setPadding("0 16px").setJustifyContent("center");
        commonStyles.addRule(".reuise-divider_label").setColor("#0000009c");
        commonStyles.addRule(".reuise-divider_label:empty").setDisplay("none");
        commonStyles.addRule(".reuise-divider--horizontal .reuise-divider_label").setPadding("0 16px");
        commonStyles.addRule(".reuise-divider--vertical .reuise-divider_label").setPadding("16px 0");
        commonStyles.addRule(".reuise-divider_startline, .reuise-divider_endline").setFlex("1 1 auto").setBackgroundColor("#0000001f");
        commonStyles.addRule(".reuise-divider--horizontal .reuise-divider_startline, .reuise-divider_endline").setHeight("1px");
        commonStyles.addRule(".reuise-divider--vertical .reuise-divider_startline, .reuise-divider_endline").setWidth("1px");
        commonStyles.addRule(((("." + createLabelAlignmentStyleClassName(Alignment.START)) + ".reuise-divider_startline, .") + createLabelAlignmentStyleClassName(Alignment.END)) + ".reuise-divider_startline").setMaxWidth(8);
    }
}