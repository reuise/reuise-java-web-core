...
import dev.reuise.core.avatar.AvatarShape;
import dev.reuise.core.avatar.AvatarSize;
import dev.reuise.core.avatar.DefaultCoreAvatarPart;
...
public abstract class DefaultWebAvatarPart<S extends DefaultWebAvatarPart<S, O>, O extends WebAvatarPartOptions> extends DefaultCoreAvatarPart<S, O> implements WebAvatar , WebComponentPart {
    @Override
    ...
    private String createSizeStyleClassName(AvatarSize size) {
    ...
    public S setShape(AvatarShape shape) {
        if (shape == null) {
            removeShapeStyleClass();
            return self();
        }
        for (AvatarShape s : AvatarShape.values()) {
            addOrRemoveStyleClass(createShapeStyleClassName(s), shape == s);
        }
        return self();
    }
    ...
    public AvatarShape getShape() {
        for (AvatarShape s : AvatarShape.values()) {
            if (hasStyleClass(createShapeStyleClassName(s)))
                return s;
    ...
    private void removeShapeStyleClass() {
        for (AvatarShape s : AvatarShape.values())
            removeStyleClass(createShapeStyleClassName(s));

    }

    private String createShapeStyleClassName(AvatarShape shape) {
        return (getBaseStyleClass() + "--") + shape.getShortName();
    }
    ...
    protected DefaultWebAvatarPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (badge != null)
            badge.setRootComponent(rootComponent);
    ...
    @Override
    public WebAvatar getComponent() {
        return ((WebAvatar) (baseComponentPart.getComponent()));
    }
    ...
    @Override
    public WebBadge getBadge() {
        return ((WebBadge) (super.getBadge()));
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        ...
        commonStyles.addRule(".reuise-avatar_image").setWidth("inherit").setHeight("inherit").setClipPath("circle(49%)");
    }
