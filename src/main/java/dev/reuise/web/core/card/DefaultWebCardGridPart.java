...
import dev.reuise.core.card.CardGridPositionMode;
...
public abstract class DefaultWebCardGridPart<S extends DefaultWebCardGridPart<S, O>, O extends WebCardGridPartOptions> extends DefaultCoreCardGridPart<S, O> implements WebCardGrid , WebComponentPart {
    ...
    public S setPositionMode(CardGridPositionMode positionMode) {
        if (positionMode == null) {
            removePositionModeStyleClass();
            return self();
        }
        for (CardGridPositionMode m : CardGridPositionMode.values()) {
            addOrRemoveStyleClass(createPositionModeStyleClassName(m), positionMode == m);
        }
        return self();
    }
    ...
    public CardGridPositionMode getPositionMode() {
        for (CardGridPositionMode m : CardGridPositionMode.values()) {
            if (hasStyleClass(createPositionModeStyleClassName(m)))
                return m;
    ...
    private void removePositionModeStyleClass() {
        for (CardGridPositionMode m : CardGridPositionMode.values())
            removeStyleClass(createPositionModeStyleClassName(m));

    }

    private String createPositionModeStyleClassName(CardGridPositionMode positionMode) {
        return (getBaseStyleClass() + "--positionmode_") + positionMode.getShortName();
    }

    @Override
    public boolean isAnimationEnabled() {
        return hasStyleClass(createAnimationEnabledStyleClassName());
    }

    @Override
    public CoreCardGridPart setAnimationEnabled(Boolean animationEnabled) {
        addOrRemoveStyleClass(createAnimationEnabledStyleClassName(), animationEnabled);
        return self();
    }

    private String createAnimationEnabledStyleClassName() {
        return getBaseStyleClass() + "--animationEnabled";
    }
    ...
    protected DefaultWebCardGridPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (cardContainer != null)
            cardContainer.setRootComponent(rootComponent);
    ...
    public S setTheme(Theme theme) {
        ...
        if (scrim != null)
            scrim.setTheme(theme);

        if (cardContainer != null)
            cardContainer.setTheme(theme);
    ...
    @Override
    public WebCardGrid getComponent() {
        return ((WebCardGrid) (baseComponentPart.getComponent()));
    }
    ...
    @Override
    public WebContainer getCardContainer() {
        return ((WebContainer) (super.getCardContainer()));
    }

    // Implementation
    // Implementation
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-cardgrid").setWidth("inherit").setMinWidth("100%").setHeight("inherit").setOverflow("auto");
        commonStyles.addRule(".reuise-cardgrid--positionmode_absolute .reuise-card").setPosition("absolute");
        commonStyles.addRule(".reuise-cardgrid:not(.reuise-cardgrid--positionmode_absolute) .reuise-cardgrid_cardcontainer").setDisplay("flex").setFlexWrap("wrap").setBoxSizing("border-box");
        commonStyles.addRule(".reuise-cardgrid--positionmode_absolute .reuise-cardgrid_cardcontainer").setPosition("relative");
        commonStyles.addRule(".reuise-cardgrid_card").setTransition("all 200ms ease-in-out");
        commonStyles.addRule(".reuise-cardgrid_card--hidden").setOpacity("0");
        commonStyles.addRule(".reuise-cardgrid_scrim--visible").setOpacity("1").setCursor("pointer").setPointerEvents("all").setZIndex(1);
        commonStyles.addRule(".reuise-cardgrid_scrim").setPosition("absolute").setBackground("#00000026").setTop("0").setRight("0").setBottom("0").setLeft("0").setOpacity(0).setPointerEvents("none").setTransition("opacity 100ms ease-in-out");
    }
    ...
    public S setCardWidthAllStates(Integer cardWidth) {
        setCardWidth(ScreenSizeValues.of(cardWidth, cardWidth));
    ...
    public S setCardHeightAllStates(Integer cardHeight) {
        setCardHeight(ScreenSizeValues.of(cardHeight, cardHeight));
