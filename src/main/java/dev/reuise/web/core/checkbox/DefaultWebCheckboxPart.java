...
import dev.reuise.core.checkbox.DefaultCoreCheckboxPart;
...
public abstract class DefaultWebCheckboxPart<S extends DefaultWebCheckboxPart<S, O>, O extends WebCheckboxPartOptions> extends DefaultCoreCheckboxPart<S, O> implements WebCheckbox , WebComponentPart {
    @Override
    public S setSize(CheckboxSize size) {
        ...
        for (CheckboxSize s : CheckboxSize.values()) {
    ...
    private String createSizeStyleClassName(CheckboxSize size) {
    ...
    protected DefaultWebCheckboxPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebCheckbox getComponent() {
        return ((WebCheckbox) (baseComponentPart.getComponent()));
    }
    ...
    @Override
    public WebBasicCheckbox getCheckbox() {
        return ((WebBasicCheckbox) (super.getCheckbox()));
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule("." + getBaseStyleClass()).setDisplay("flex").setAlignItems("center");
        commonStyles.addRule(("." + getBaseStyleClass()) + " .reuise-checkbox_label").setMarginLeft(8);
        for (CheckboxSize s : CheckboxSize.values()) {
            commonStyles.addRule(("." + createSizeStyleClassName(s)) + " .reuise-checkbox_label").setFontSize(s.getFontSize()).setLineHeight(s.getSize());
        }
    }
}