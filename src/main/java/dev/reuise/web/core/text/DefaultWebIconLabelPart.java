...
import dev.reuise.core.text.DefaultCoreIconLabelPart;
...
public abstract class DefaultWebIconLabelPart<S extends DefaultWebIconLabelPart<S, O>, O extends WebIconLabelPartOptions> extends DefaultCoreIconLabelPart<S, O> implements WebIconLabel , WebComponentPart {
    @Override
    ...
    protected DefaultWebIconLabelPart(O options) {
        super(options);
    }
    ...
    @Override
    public WebIconLabel getComponent() {
        return ((WebIconLabel) (baseComponentPart.getComponent()));
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-iconlabel,.reuise-iconlabel>a").setDisplay("flex").setFlexDirection("row").setJustifyContent("start").setAlignItems("center").setColumnGap(16);
    }
}