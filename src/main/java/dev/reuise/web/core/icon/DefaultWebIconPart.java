...
public abstract class DefaultWebIconPart<S extends DefaultWebIconPart<S, O>, O extends WebIconPartOptions> extends DefaultCoreIconPart<S, O> implements WebIcon , WebComponentPart {
    ...
    protected WebSvg svg;

    protected DefaultWebIconPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (svg != null)
            svg.setRootComponent(rootComponent);

        if (link != null)
            link.setRootComponent(rootComponent);
    ...
    @Override
    public WebIcon getComponent() {
        return ((WebIcon) (baseComponentPart.getComponent()));
    }
    ...
    private WebSvg createSvg(O options) {
        ...
        WebLinkPart link = ((WebLinkPart) (getLink()));
        WebParentComponentPart parent = (link != null) ? link : getParentComponentPart();
        svgOptions.setParent(link != null ? ((CoreComponent) (link)) : this);
        ComponentCreator svgCreator = options.getSvgCreator();
        svg = (svgCreator != null) ? ((WebSvg) (svgCreator.create(svgOptions))) : getComponentFactory().createSvg(svgOptions);
        parent.add(svg);
        return svg;
    }
    ...
    public void applyOptions(O options, OptionApplicator applicator, Collection<State> states) {
        ...
        WebIconPart component = ((WebIconPart) (options.getComponent()));
        if (component == null)
            System.out.println("component is null");

        // Apply options for default state
        applicator.add(options.getAltTextOption(), component::setAltText);
    }
    ...
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-icon_image").setDisplay("block");
        commonStyles.addRule(".reuise-icon>*").setWidth("inherit").setHeight("inherit");
        commonStyles.addRule(".reuise-icon svg:empty").setDisplay("none");
    }
}