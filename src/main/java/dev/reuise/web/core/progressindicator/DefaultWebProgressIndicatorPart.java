...
import dev.reuise.core.progressindicator.DefaultCoreProgressIndicatorPart;
import dev.reuise.core.progressindicator.ProgressIndicatorLayout;
...
public abstract class DefaultWebProgressIndicatorPart<S extends DefaultWebProgressIndicatorPart<S, O>, O extends WebProgressIndicatorPartOptions> extends DefaultCoreProgressIndicatorPart<S, O> implements WebProgressIndicator , WebComponentPart {
    @Override
    public S setLayout(ProgressIndicatorLayout layout) {
        addOrRemoveStyleClass("reuise-progressindicator--" + layout.name().toLowerCase(), true);
        return self();
    }

    @Override
    public ProgressIndicatorLayout getLayout() {
        for (ProgressIndicatorLayout layout : ProgressIndicatorLayout.values()) {
            if (hasStyleClass("reuise-progressindicator--" + layout.name().toLowerCase()))
                return layout;
    ...
    public void start() {
        super.start();
        addOrRemoveStyleClass("reuise-progressindicator--started", true);
    }

    @Override
    public void stop() {
        super.stop();
        addOrRemoveStyleClass("reuise-progressindicator--started", false);
    }

    @Override
    public boolean isStarted() {
        return hasStyleClass("reuise-progressindicator--started");
    }
    ...
    protected DefaultWebProgressIndicatorPart(O options) {
        super(options);
    }
    ...
    public void setRootComponent(RootComponent rootComponent) {
        ...
        if (track != null)
            track.setRootComponent(rootComponent);
    ...
    @Override
    public WebProgressIndicator getComponent() {
        return ((WebProgressIndicator) (baseComponentPart.getComponent()));
    }
    ...
    @Override
    public WebContainer getTrack() {
        return ((WebContainer) (super.getTrack()));
    }

    // Implementation
    // Implementation
    public void onInitializeCommonStyle(StyleBuilder commonStyles) {
        commonStyles.addRule(".reuise-progressindicator").setOverflow("hidden");
        commonStyles.addRule(".reuise-progressindicator--started").setDisplay("block");
        commonStyles.addRule(".reuise-progressindicator--linear").setWidth("100%").setHeight("6px").setDisplay("none");
        commonStyles.addRule(".reuise-progressindicator_track").setPosition("relative").setAnimationIterationCount("infinite").setAnimationTimingFunction("linear").setAnimationDuration("3s").setAnimationName("reuise-progressindicator_animation").setMaxWidth("33%").setHeight("inherit").setBackground("linear-gradient(to right, transparent 0%, #00f5ff 50%, transparent 100%)");
        StyleSheet keyframes = commonStyles.addAtRule("keyframes reuise-progressindicator_animation");
        keyframes.addRule("0%").setLeft("-33%");
        keyframes.addRule("100%").setLeft("100%");
    }
}